...
class DescribedFeatureOverlay extends StatefulWidget {
  ...
  @override
  DescribedFeatureOverlayState createState() =>
      DescribedFeatureOverlayState();
}

class DescribedFeatureOverlayState extends State<DescribedFeatureOverlay>
    ...
    if (isBackgroundCentered) {
      ...
      switch (contentLocation) {
        case ContentLocation.above:
          final rawX = anchor.dx -
              width / 2.0 +
              (_isOnLeftHalfOfScreen(anchor) ? -20.0 : 20.0);
          // CUSTOM FIX: Clamp X to keep background circle on-screen
          // The circle needs at least width/2 space from the left edge
          final clampedX = max(width / 2.0, rawX);
